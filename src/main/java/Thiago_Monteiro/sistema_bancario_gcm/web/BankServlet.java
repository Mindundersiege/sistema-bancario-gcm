...
public class BankServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		/*
		 * if (person != null) { String json = "{\n"; json += "\"name\": " +
		 * JSONObject.quote(person.getName()) + ",\n"; json += "\"about\": " +
		 * JSONObject.quote(person.getAbout()) + ",\n"; json +=
		 * "\"birthYear\": " + person.getBirthYear() + "\n"; json += "}";
		 * response.getOutputStream().println(json); } else { // That person
		 * wasn't found, so return an empty JSON object. We // could also return
		 * an error. response.getOutputStream().println("{}"); }
		 */
	}
